...
module.exports = async () => {
    ...
    (async () => {
        ...
        await page.goto('https://www.comune.valdagno.vi.it/eventi/eventi-1', { waitUntil: 'networkidle2' });
        ...
        const scrapeData = async () => {
            const data = await page.evaluate(() => {
                const items = Array.from(document.querySelectorAll('.eventsListing')); // Modifica il selettore
                return items.map(item => ({// Modifica il selettore
                    link: item.querySelector('.eventListTitle a').href // Modifica il selettore se necessario
                }));
            });
            return data;
        };
        ...
        for (const item of allData) {
            ...
            const eventData = await eventPage.evaluate(() => {
                ...
                // const id = String(bodyClass).split('-')[1] ?? null;


                // const category = document.querySelector('.category')?.innerText ?? null; 

                // const h1Element = document.querySelector('h1');
                // const title = h1Element?.innerText ?? null; 
                // const description = document.querySelector('.event_body')?.innerText ?? null; 
                // const dataRange = document.querySelector('.col-12.order-1.order-md-12.bg-eventi.uppercase')?.innerText ?? null;
                // let dataInizio = dataRange.split(': ')[1] ?? null;
                // let dataFine = dataRange.split(': ')[2] ?? null;
                // dataInizio = moment(dataInizio.split('\n')[0], "DD MMM HH:mm");
                // dataFine = moment(dataFine.split('\n')[0], "DD MMM HH:mm")

                // let place = document.querySelector('.m-hero-show-info')?.innerText ?? null;
                // place = place.split('\n')[1]
                // const data = {
                //     start: dataInizio.format('YYYY-MM-DD HH:mm')?? null,
                //     end: dataFine.format('YYYY-MM-DD HH:mm')?? null,
                // }
                // const image = document.querySelector('img.img').src ?? null; 
